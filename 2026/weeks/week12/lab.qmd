---
title: "Práctico 8: Introducción al análisis bivariado: explorando el uso del tiempo con la ENUT 2023"
subtitle: "Trabajando con la Encuesta Nacional de Uso del Tiempo (ENUT) 2023"
date: "2026-11-16"
lang: es
format:
  html:
...
```{r cat-quant-vis}
#| fig-alt: "Boxplots comparando las horas diarias de trabajo no remunerado entre hombres y mujeres. La mediana de las mujeres (4,11 horas) supera el tercer cuartil de los hombres (3,96 horas): más de la mitad de las mujeres dedica más tiempo a estas tareas que el 75% de los hombres. La mediana de los hombres es 2,10 horas."
...
```{r cat-cat-vis}
#| fig-alt: "Gráfico de barras apiladas al 100% con el nivel de satisfacción por el reparto de tareas domésticas, según sexo. Los hombres reportan mayor satisfacción: 28,3% está 'Totalmente satisfecho' versus 21,2% de las mujeres. La insatisfacción combinada (insatisfecho más totalmente insatisfecho) es de 6,2% en hombres y 14,8% en mujeres."
enut_limpio_cat %>%
  ggplot(aes(x = sexo_factor, fill = fct_rev(bs2_factor), weight = fe_cut)) +
  geom_bar(position = "fill", width = 0.6) +
  scale_y_continuous(labels = scales::percent) +
  scale_fill_manual(
    values = c("#1E2240", "#7C90CC", "#B8B8B8", "#D9A594", "#C2694B")
  ) +
  labs(
...
```{r quant-quant-vis, warning=FALSE}
#| fig-alt: "Gráfico de dispersión de horas de trabajo doméstico no remunerado según edad (12 a 100 años), con dos líneas de tendencia superpuestas: una curva loess en rojo y una recta de regresión lineal en azul. La curva loess muestra una relación curvilínea, con el tiempo aumentando hasta la adultez media y disminuyendo después, mientras la línea lineal capta una asociación positiva débil (correlación de 0,20)."
...
**Interpretación:** La correlación es `r round(cor(enut_trabajo$edad, enut_trabajo$t_tdnr_dt, use = "pairwise.complete.obs"), 2)`, un valor bajo que confirma que la relación **no es lineal**, como ya habíamos visto en el gráfico.
