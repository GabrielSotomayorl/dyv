...
Esto nos permite usar **rutas relativas** (ej. `"datos/Base de datos Casen 2024 SPSS.sav"`) en lugar de rutas absolutas (ej. `"C:/Users/Gabriel/Documentos/Curso/datos/Base de datos Casen 2024 SPSS.sav"`), lo que asegura que nuestro código funcione en cualquier computador.
...
La base de datos está disponible en el sitio del [Observatorio Social del Ministerio de Desarrollo Social y Familia](https://observatorio.ministeriodesarrollosocial.gob.cl/encuesta-casen-2024) en formato **SPSS (`.sav`)**. Descárgala y guárdala dentro de la carpeta de tu proyecto (por ejemplo, en una subcarpeta llamada `datos`). Luego cárgala así:

```{r load-show, eval=FALSE}
casen <- haven::read_sav("datos/Base de datos Casen 2024 SPSS.sav")
```

::: {.callout-note title="¿Por qué usamos `haven` para cargar los datos?"}
Las encuestas oficiales suelen distribuirse en formatos de otros programas estadísticos, como SPSS (`.sav`) o Stata (`.dta`). El paquete `haven` permite leerlos directamente en R y —esto es lo importante— **conserva las etiquetas** de las variables: pequeñas notas que indican qué significa cada código (por ejemplo, que en `sexo` el `1` es *"Hombre"* y el `2` es *"Mujer"*). Más adelante usaremos `as_factor()` para convertir esos códigos numéricos en categorías con nombre. Si abriéramos los datos con una función que no lee etiquetas, veríamos solo números sin saber qué significan.
:::
...
`haven` ha importado las variables numéricas de SPSS (ej. `sexo` donde 1="Hombre", 2="Mujer") y también ha guardado las **etiquetas de texto** asociadas. Por eso vemos `dbl+lbl` o `haven_labelled`. Por defecto, R las trata como números, pero guarda las etiquetas. Para que nuestro análisis sea más claro, necesitamos "activar" esas etiquetas.
...
**Actividad de Interpretación:** Basado en la tabla de frecuencias porcentuales, ¿qué porcentaje de las personas en la muestra vive en condiciones de hacinamiento (sumando las categorías de hacinamiento medio, alto y crítico)?
