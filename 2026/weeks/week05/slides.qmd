...
title: "<font style='font-size:1em;'>🗓️ Semana 05<br/> La estructura de los datos en R</font>"
...
## Conectando con la unidad anterior {.smaller}
...
![](img/data-science-explore.svg){fig-alt='Diagrama del ciclo de la ciencia de datos: un flujo lineal de Importar a Ordenar (Tidy), que entra a un ciclo central llamado Explorar donde se conectan Transformar, Visualizar y Modelar, y termina en Comunicar. Todo el proceso está enmarcado dentro de un recuadro más grande llamado Programar, indicando que la programación es la base que sostiene cada etapa.'}


# 2. Los tipos de datos en R

## Tipos de datos fundamentales {.smaller}
...
## El concepto de coerción {.smaller}
...
## Estructuras de datos: Los contenedores de información {.smaller}
...
## 1. El vector: La variable individual {.smaller}
...
## 2. La lista: El contenedor flexible {.smaller}
...
## 3. La matriz y el data frame {.smaller}
...
## 4. El data frame: Nuestra tabla de datos estándar {.smaller}
...
## La lógica de una tabla de datos {.smaller}
...
## El ideal: Datos ordenados (tidy data) {.smaller}
...
![](img/tidy-1.svg){fig-alt='Ilustración de las tres reglas de los datos ordenados (tidy data) sobre una tabla de ejemplo con país (Afganistán, Brasil, China), año (1999 y 2000), casos y población. Un recuadro resalta las columnas y las etiqueta "variables"; otro resalta las filas y las etiqueta "observaciones"; un tercero resalta las celdas individuales y las etiqueta "valores".'}

Afortunadamente, la mayoría de las encuestas (como la CASEN o la ENE) ya vienen en este formato. Nuestro trabajo es mantener este orden.

# 3. Un flujo de trabajo para explorar datos

## Buenas prácticas: Script vs. consola {.smaller}
...
:::
::: {.column width="50%"}
**El Script (.R) o el documento Quarto (.qmd)**  
...
-   **Siempre debemos trabajar en un script o en un documento Quarto.**  
:::
::::::
...
## Buenas prácticas para un código reproducible {.smaller}
...
## La exploración inicial de los datos {.smaller}
...
## Descripción básica: `summary()` y `table()` {.smaller}
