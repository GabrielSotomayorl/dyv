...
title: "<font style='font-size:1em;'>🗓️ Semana 09<br/> Visualizando Distribuciones y Medidas Robustas</font>"
...
En la clase anterior describimos **variables categóricas**:

-   Construimos tablas de frecuencias absolutas y relativas con `table()` y `count()`.
-   Incorporamos **factores de expansión** para estimar proporciones en la población.
-   Visualizamos la distribución con **gráficos de barras** (`barplot()` y `geom_bar()`).

**Hoy** pasamos a las **variables cuantitativas** (de intervalo o razón), como la edad, los ingresos o los años de escolaridad. Con estas variables, un simple conteo no es suficiente: necesitamos herramientas para describir la **forma**, el **centro** y la **dispersión** de su distribución.


# 1. Descripción de Variables Cuantitativas

## De las Categorías a los Números {.smaller}

Pasamos de las variables categóricas a las **variables cuantitativas** (de intervalo o razón), como la edad, los ingresos o los años de escolaridad.
...
## El Histograma: Un Retrato de la Distribución {.smaller}
...
## Ingreso del Trabajo Principal en Chile {.smaller}
...
## La Fuente de Datos: Encuesta Suplementaria de Ingresos (ESI) {.smaller}
...
## Interpretando la Forma: Asimetría (Sesgo) {.smaller}
...
ggplot(datos_asimetria, aes(x = valor)) +
  ...
  scale_fill_manual(values = c("#7C90CC", "#C2694B", "#1E2240")) +
...
## Interpretando la Forma: Modalidad {.smaller}
...
ggplot(datos_modalidad, aes(x = valor)) +
  geom_histogram(aes(fill = tipo), color = "white", bins = 30, show.legend = FALSE) +
  scale_fill_manual(values = c("#7C90CC", "#C2694B")) +
...
## ¿Qué es una Medida Robusta? {.smaller}
...
## La Mediana (M o Q2): El Centro Geométrico {.smaller}
...
## Cuartiles y Rango Intercuartil (IQR) {.smaller}
...
## Percentiles: Más Allá de los Cuartiles {.smaller}
...
## Ejemplo: Los Quintiles de Ingreso {.smaller}
...
## El Resumen de Cinco Números y el Boxplot {.smaller}
...
## El Resumen de Cinco Números en la Práctica {.smaller}
...
## La Anatomía de un Boxplot {.smaller}
...
ggplot(df_boxplot, aes(x = "", y = y)) +
  geom_boxplot(fill = "#7C90CC", width = 0.22, alpha = 0.85, linewidth = 0.8, outlier.size = 2.8) +
...
## El Boxplot  de Ingreso del Trabajo Principal {.smaller}
...
esi %>%
  ...
  geom_boxplot(fill = "#1E2240", outlier.color = "#7C90CC", outlier.alpha = 0.3) +
