---
title: "Práctico 10: Análisis cuantitativo bivariado: dispersión y correlación"
...
#| fig-alt: "Gráfico de dispersión entre edad y horas diarias de trabajo doméstico no remunerado, con dos líneas de tendencia: una recta de regresión lineal (roja) y una curva loess (azul discontinua). La línea lineal muestra una pendiente positiva suave (correlación r=0,204). La curva loess revela un patrón más rico: el trabajo doméstico aumenta rápidamente entre los 15 y 30 años, se mantiene alto y estable durante la adultez, y decae levemente en la vejez avanzada, un patrón que la línea recta no logra capturar."
...
ggplot(enut_cuant, aes(x = edad, y = t_tdnr_dt)) +
  ...
  geom_point(alpha = 0.1, color = "#1E2240") +
  
  # Capa 2: Tendencia Lineal 
  # method = "lm" fuerza una línea recta (Linear Model)
  geom_smooth(method = "lm", color = "#C2694B", se = FALSE, linewidth = 1) +
  
  # Capa 3: Tendencia Local
  # method = "loess" se adapta a las curvas de los datos
  geom_smooth(method = "loess", color = "#4A8B8A", se = FALSE, linetype = "dashed", linewidth = 1) +
...
```{r corrr-plot}
#| fig-alt: "Mapa de calor con la matriz de correlaciones entre edad, horas de trabajo remunerado, trabajo doméstico no remunerado y trabajo de cuidados no remunerado. Las correlaciones más notorias son: trabajo doméstico y trabajo de cuidados (r=0,20, positiva, en azul), trabajo remunerado y trabajo doméstico (r=-0,20, negativa, en rojo), y edad con trabajo doméstico (r=0,20, positiva). El resto son débiles y cercanas a cero: edad con trabajo remunerado (0,02), edad con cuidados (-0,15) y trabajo remunerado con cuidados (-0,15)."
matriz_tidy %>%
  rplot(colors = c("#C2694B", "white", "#1E2240")) + # Terracota (neg) - Blanco - Azul noche (pos)
