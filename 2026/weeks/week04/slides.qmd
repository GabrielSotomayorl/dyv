...
title: "<font style='font-size:1em;'>🗓️ Semana 04<br/> Conceptos clave para la investigación social II</font>"
...
## Repaso: De la Idea al Dato {.smaller}
...
## ¿Por qué Muestrear? Población vs. Muestra {.smaller}
...
:::
::: {.column width="35%"}
![](img/01.png){fig-alt='Ilustración conceptual con figuras humanas de colores: un círculo grande con muchas personas de distintos colores, etiquetado "Población", del cual una flecha señala a un círculo pequeño con solo cinco de esas personas, etiquetado "Muestra", mostrando que la muestra es un subconjunto de la población.' fig-align="center"}
:::
::::::

## La Lógica del Muestreo Probabilístico {.smaller}
...
## Población Objetivo vs. Marco Muestral {.smaller}
...
## Ejemplo 1: Encuesta CASEN {.smaller}
...
## Ejemplo 2: Encuesta CADEM Plaza Pública {.smaller}
...
## Ejemplo 3: Encuesta Longitudinal de Primera Infancia (ELPI) {.smaller}
...
## El Ideal: Muestreo Aleatorio Simple (MAS) {.smaller}
...
## ¿Por qué el Muestreo Aleatorio Simple es tan raro en la práctica? {.smaller}
...
## Solución Práctica 1: Muestreo Estratificado {.smaller}
...
## Solución Práctica 2: Muestreo por Conglomerados {.smaller}
...
## Poniendo todo junto: Diseños Muestrales Complejos (Multietápicos) {.smaller}
...
## De la Selección a la Calidad {.smaller}
...
## Fiabilidad: ¿Es consistente nuestra medición? {.smaller}
...
## Validez: ¿Medimos lo que realmente queremos medir? {.smaller}
...
## Tres Tipos de Validez: ¿Cómo evaluamos la exactitud? {.smaller}
...
## La Analogía del Tiro al Blanco {.smaller}

Una forma clásica de entender la diferencia es con la analogía de un tirador:

![](img/03.png){fig-alt='Cuatro dianas de tiro al blanco que ilustran fiabilidad y validez: "Fiable pero no válido" (impactos agrupados pero lejos del centro), "Válido pero no fiable" (impactos dispersos alrededor del centro), "Ni válido ni fiable" (impactos dispersos y lejos del centro), y "Válido y fiable" (impactos agrupados en el centro).' fig-align="center"}
...
## La Tensión entre Fiabilidad y Validez {.smaller}
...
## El Error Total de Encuesta: Un Marco Integrador {.smaller}
...
## {}
![](img/04.png){fig-alt='Diagrama del marco del Error Total de Encuesta, con dos cadenas paralelas de recuadros conectados por flechas. Rama de medición (izquierda): Constructo, luego Medición (con Error de Validez), luego Respuesta (con Error de Medición), luego Respuesta editada (con Error de Procesamiento). Rama de representación (derecha): Población objetivo, luego Marco muestral (con Error de Cobertura), luego Muestra (con Error de Muestreo), luego Muestra lograda (con Error de No Respuesta), luego Ajuste post-encuesta (con Error de Ajuste). Ambas ramas convergen en el recuadro final "Estadístico de encuesta".' fig-align="center"}


## El Error Total como un Desafío de Diseño {.smaller}
...
## Actividad de Aplicación (15 min) {.smaller}
...
## Cierre de la Unidad 2 y Próximos Pasos {.smaller}
