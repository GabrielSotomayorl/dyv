...
title: "<font style='font-size:1em;'>🗓️ Semana 11<br/> La visualización como argumento sociológico y la gramática de gráficos</font>"
...
casen <- casen %>% mutate(pobreza_factor = as_factor(pobreza))
```
...
En la clase anterior completamos la descripción numérica de variables cuantitativas:

-   La **media** como "centro de gravedad" de los datos, sensible a valores atípicos.
-   La comparación **Media vs. Mediana** como diagnóstico de asimetría: si la media > mediana, la distribución tiene cola a la derecha.
-   La **desviación estándar** como la "distancia típica" de los datos respecto a la media.
-   Las **Puntuaciones Z** para comparar observaciones de diferentes distribuciones en una escala universal.

**Hoy** cerramos la Unidad 4 aprendiendo a *mostrar* esas distribuciones de manera efectiva: la visualización no como decoración, sino como argumento.


# 1. La visualización como argumento sociológico

## Más allá de la ilustración {.smaller}
...
## La curva en U de Piketty (la desigualdad en el tiempo) {.smaller}

![](img/01.jpg){fig-alt='Gráfico de línea de tiempo (Gráfica 1 de Piketty): participación del decil superior en el ingreso nacional de Estados Unidos, 1910-2010. Dibuja una curva en forma de U: pasa de 45-50% en 1910-1920 a menos de 35% en la década de 1950 (la caída documentada por Kuznets), y vuelve a subir a 45-50% entre 2000 y 2010.' fig-align="center"}
...
## El espacio social de Bourdieu (la desigualdad en el espacio) {.smaller}

![](img/02.jpg){fig-alt='Mapa detallado de Bourdieu "The space of social positions": un plano con el volumen de capital total en el eje vertical (más arriba, menos abajo) y la composición del capital en el horizontal (capital cultural alto/capital económico bajo a la izquierda; capital económico alto/capital cultural bajo a la derecha). Decenas de categorías ocupacionales (profesores, ejecutivos, industriales, obreros) y prácticas culturales asociadas (ópera, golf, ajedrez, esquí acuático, camping) se ubican como puntos en este espacio, mostrando cómo cada posición social combina un perfil propio de capital y de gustos.' fig-align="center"}
...
## Veamos algunos otros ejemplos {.smaller}

![](img/03.png){fig-alt='Gráfico de Our World in Data "Share of the world population living in poverty" (1820-2025), con varias líneas descendentes. La pobreza extrema histórica (Moatsos, morado) cae de 79% en 1820 a 9% en 2018. Con datos recientes del Banco Mundial (verde), la población bajo distintos umbrales diarios cae entre 1990 y 2025: bajo 3 dólares de 44% a 10%, bajo 5 dólares de 60% a 24%, bajo 10 dólares de 73% a 52%, y bajo 30 dólares de 87% a 81%.' fig-align="center"}


## Veamos algunos otros ejemplos {.smaller}

![](img/04.png){fig-alt='Gráfico 5 (Corak, 2016): elasticidad intergeneracional del ingreso versus desigualdad (coeficiente Gini), por país. Muestra una relación positiva: países más desiguales como Perú, Brasil y Chile tienen mayor elasticidad (menor movilidad social entre generaciones), mientras países más igualitarios como Dinamarca, Noruega y Finlandia tienen baja elasticidad (mayor movilidad). Chile aparece entre los países con más desigualdad y menor movilidad, junto a Argentina y Brasil.' fig-align="center"}



## Veamos algunos otros ejemplos {.smaller}

![](img/05.jpg){fig-alt='Gráfico de barras apiladas "Incidencia de la pobreza y la pobreza extrema en la población, 2006-2022" en Chile. La pobreza total cae sostenidamente de 28,7% en 2006 a 6,5% en 2022, con un repunte puntual a 10,7% en 2020. La pobreza extrema baja de 12,4% en 2006 a 2,0% en 2022.' fig-align="center"}


## Veamos algunos otros ejemplos {.smaller}

![](img/06.jpg){fig-alt='Gráfico 7: segmentación social de las escuelas en Chile y resultados de PISA 2012 — puntaje PISA versus índice de nivel socioeconómico y cultural, por país y, dentro de Chile, por tipo de establecimiento. Los colegios particulares privados chilenos obtienen el puntaje más alto (cerca de 545, similar a países como Finlandia) con alto nivel socioeconómico, seguidos por los particulares subvencionados y, muy por debajo, los municipales (cerca de 440), evidenciando una fuerte segmentación social del sistema escolar chileno.' fig-align="center"}



# 2. La visualización como diagnóstico estadístico

## El engaño de los estadísticos descriptivos {.smaller}
...
## ¡GRAFICA SIEMPRE TUS DATOS! {.smaller}
...
## Principios para gráficos claros y honestos {.smaller}
...
## La regla de oro del eje cero {.smaller}
...
p1 <- ggplot(data_barras, aes(x = grupo, y = valor)) + geom_col(fill = "#7C90CC") + labs(title = "Honesto: Eje Y empieza en 0") + theme_minimal(base_size=14)
p2 <- ggplot(data_barras, aes(x = grupo, y = valor)) + geom_col(fill = "#C2694B") + coord_cartesian(ylim = c(80, 105)) + labs(title = "Engañoso: Eje Y truncado") + theme_minimal(base_size=14)
...
## Elige el gráfico adecuado {.smaller}
...
## Un sistema para construir gráficos en capas {.smaller}
...
## Los 3 componentes esenciales {.smaller}
...
## La plantilla universal de `ggplot2` {.smaller}
...
## Construyendo por capas: Ejemplo {.smaller}
...
## Mapear vs. fijar: Ejemplo visual {.smaller}
...
## Ampliando la gramática: Faceting (small multiples) {.smaller}
...
ggplot(mtcars, aes(x = mpg)) +
  geom_histogram(fill = "#7C90CC", color = "white", bins = 5) +
...
-   Aplicarán estos principios para diagnosticar datos, construir sus primeros gráficos con `ggplot2` desde cero y usar `facet_wrap` para análisis comparativos con la CASEN 2024.
