---
title: "Práctico 6: La Media, la Desviación Estándar y la Estandarización en R"
...
n_casos <- sum(!is.na(esi_trabajo$edad))
# 3. Dividimos
...
```
**Interpretación:** En el histograma de horas, la línea de la media (roja) aparece a la izquierda de la mediana (azul), lo que señala **asimetría a la izquierda**. En el gráfico de densidad del ingreso ocurre lo contrario: la media queda muy a la derecha de la mediana, señal de **asimetría a la derecha** causada por ingresos extremadamente altos en la cola superior. El gráfico de ingresos está acotado al percentil 99 para poder visualizar la distribución central; los valores más extremos quedan fuera.

-   **Horas trabajadas:** La media (`r round(mean(esi_trabajo$efectivas, na.rm = T),1)`) es menor que la mediana (`r round(median(esi_trabajo$efectivas, na.rm = T),1)`), lo que confirma la **asimetría a la izquierda**.
-   **Ingreso:** La media (`r format(round(mean(esi_trabajo$ing_t_p, na.rm = T)), big.mark = ".")`) es mucho mayor que la mediana (`r format(round(median(esi_trabajo$ing_t_p, na.rm = T)), big.mark = ".")`), confirmando la **asimetría a la derecha**.


```{r warning=FALSE, message=FALSE}
#| fig-alt: "Histograma de las horas efectivas trabajadas por semana, con líneas verticales marcando la media (37,0 horas, línea roja discontinua) y la mediana (44 horas, línea azul sólida). La media queda a la izquierda de la mediana, evidenciando asimetría a la izquierda."
...
ggplot(esi_trabajo, aes(x = efectivas)) +
  geom_histogram(fill = "#7C90CC", color = "white") +
  geom_vline(aes(xintercept = mean_efectivas, color = "Media"), linetype = "dashed") +
  geom_vline(aes(xintercept = median_efectivas, color = "Mediana")) +
  scale_color_manual(name = "Estadístico", values = c("Media" = "#C2694B", "Mediana" = "#1E2240")) +
  labs(
...
```{r warning=FALSE}
#| fig-alt: "Gráfico de densidad del ingreso del trabajo principal, acotado al percentil 99 (bajo $3.946.528), con líneas verticales para la media ($758.430, línea roja discontinua) y la mediana ($560.000, línea azul sólida). La media queda muy a la derecha de la mediana, mostrando fuerte asimetría a la derecha."
# Plot para 'ing_t_p' (ingreso del trabajo principal, acotado al percentil 99)
mean_ing_t_p <- mean(esi_trabajo$ing_t_p, na.rm = TRUE)
median_ing_t_p <- median(esi_trabajo$ing_t_p, na.rm = TRUE)

esi_trabajo %>%
  filter(ing_t_p < quantile(ing_t_p, 0.99, na.rm = TRUE)) %>%
  ggplot(aes(x = ing_t_p)) +
  geom_density(fill = "#7C90CC", color = "#1E2240", alpha = 0.7) +
  geom_vline(aes(xintercept = mean_ing_t_p, color = "Media"), linetype = "dashed") +
  geom_vline(aes(xintercept = median_ing_t_p, color = "Mediana")) +
  scale_color_manual(name = "Estadístico", values = c("Media" = "#C2694B", "Mediana" = "#1E2240")) +
  scale_x_continuous(labels = scales::label_dollar(prefix = "$", big.mark = ".")) +
  labs(
    title = "Distribución del Ingreso del Trabajo Principal (hasta percentil 99)",
...
**Actividad 1:** Usando `summarise()` y `weighted.mean()`, calcula la media ponderada de `edad` para dos grupos: personas que trabajan a tiempo completo (`efectivas >= 40`) y personas que trabajan menos horas (`efectivas < 40`). ¿Difieren en edad promedio? ¿Cómo interpretas esa diferencia sociológicamente?
