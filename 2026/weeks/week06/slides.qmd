...
title: "<font style='font-size:1em;'>🗓️ Semana 06<br/> La gramática de la manipulación de datos</font>"
...
## El ecosistema tidyverse: Un lenguaje para la ciencia de datos {.smaller}
...
## `dplyr`: La gramática de la manipulación de datos {.smaller}
...
## El operador pipe `%>%`: Escribiendo código como se piensa {.smaller}
...
## Los 5 verbos principales {.smaller}
...
## `select()`: Trabajando con columnas {.smaller}
...
## La lógica del filtrado: Operadores de comparación {.smaller}
...
## Combinando condiciones: Operadores lógicos {.smaller}
...
## `filter()`: Aplicando la lógica para seleccionar filas {.smaller}
...
## `arrange()`: Reordenando filas {.smaller}
...
## `mutate()`: Creando y modificando variables {.smaller}
...
## `summarise()`: Colapsando datos en resúmenes {.smaller}
...
## Obteniendo estadísticos por grupo {.smaller}
...
## Calculando estadísticas por grupo {.smaller}
...
## Análisis por múltiples grupos {.smaller}
...
-   Aplicarán todos estos verbos para filtrar, transformar y resumir la Encuesta CASEN 2024, respondiendo a preguntas sociológicas específicas.
