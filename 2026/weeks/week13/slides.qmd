...
title: "<font style='font-size:1em;'>🗓️ Semana 13<br/> Tablas de contingencia y el control por tercera variable</font>"
...
-   **Operacionalizar la independencia** comparando frecuencias observadas con las esperadas si no hubiera asociación.
...
En la clase anterior analizamos el caso **Categórica (X) → Cuantitativa (Y)**:

-   Usamos `group_by()` + `summarise()` para calcular medias y medianas ponderadas por grupo.
-   Los **boxplots comparativos** (`geom_boxplot`) son la visualización estándar para este tipo de relación.
-   Aplicamos **correlación de Pearson** para medir la fuerza de la relación entre dos variables cuantitativas.
-   El `facet_wrap()` nos permite comparar la distribución de Y a través de múltiples grupos de X en una sola figura.

**Hoy** nos centramos en el caso **Categórica (X) → Categórica (Y)**, donde la herramienta principal es la **tabla de contingencia**.


# 1. La tabla de contingencia

## De comparar grupos a cruzar categorías {.smaller}
...
## Anatomía de una tabla de contingencia {.smaller}
...
## Tres tipos de porcentajes {.smaller}
...
## Ejemplo de comparación {.smaller}
...
## Operacionalizando la independencia: ¿Qué tabla esperaríamos? {.smaller}

Decir que dos variables son **independientes** tiene una traducción concreta: la distribución de Y sería **la misma en cada categoría de X**. Eso nos permite construir la tabla que **esperaríamos ver si no hubiera asociación** (repartiendo cada total de fila en proporción al tamaño de cada columna) y compararla con la que **observamos**:

```{r, echo=FALSE, message=FALSE, warning=FALSE}
tabla_obs <- enut_trabajo %>%
  filter(!is.na(p_tdnr_dt_factor)) %>%
  mutate(across(c(p_tdnr_dt_factor, sexo_factor), droplevels)) %>%
  with(table(p_tdnr_dt_factor, sexo_factor))

tabla_esp <- round(outer(rowSums(tabla_obs), colSums(tabla_obs)) / sum(tabla_obs))

bind_cols(
  tibble(`Participa en TDNR` = rownames(tabla_obs)),
  as_tibble(unclass(tabla_obs)) %>% rename_with(~paste("Observado:", .x)),
  as_tibble(unclass(tabla_esp)) %>% rename_with(~paste("Esperado:", .x))
) %>%
  kable(caption = "Frecuencias observadas vs. esperadas bajo independencia") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```

**Lectura:** si el sexo no tuviera relación con el trabajo doméstico, esperaríamos unos **453 hombres** que no participan; observamos **739** — un 60% más. La **distancia entre lo observado y lo esperado** es la manera precisa de decir "hay asociación", y es la idea sobre la que se construye el test de chi-cuadrado que verán en el curso de inferencia.


# 3. El control por una tercera variable

## ¿Es "real" la relación? {.smaller}
...
## Un ejemplo clásico: La asociación espuria {.smaller}
...
## Aplicando el control en la práctica (ENUT 2023) {.smaller}
...
## Conclusión del análisis de control {.smaller}
...
## Patrones del análisis de control {.smaller}
