---
title: "Práctico 9: Análisis categórico bivariado: tablas de contingencia y control"
...
```{r vis-direct}
#| fig-alt: "Gráfico de barras apiladas al 100% con el nivel de satisfacción por el reparto de tareas domésticas, según sexo. Los hombres reportan mayor satisfacción: 28,3% 'Totalmente satisfecho' versus 21,2% de las mujeres, mientras la insatisfacción combinada es de 6,2% en hombres y 14,8% en mujeres."
enut_practico %>%
...
```{r vis-tabular}
#| fig-alt: "El mismo gráfico de satisfacción por sexo del ejemplo anterior, esta vez construido pre-tabulando los porcentajes con dplyr y graficando con geom_col(): los hombres reportan mayor satisfacción (28,3% 'Totalmente satisfecho') que las mujeres (21,2%), con mayor insatisfacción combinada en mujeres (14,8%) que en hombres (6,2%)."
...
enut_practico %>%
  ...
  kable(caption = "Tabla 2: Participación en Cuidados por Condición de Actividad (Mujeres)") %>%
  kable_styling(bootstrap_options = "striped", full_width = FALSE)
```
...
```{r control-facet}
#| fig-alt: "Gráfico de barras apiladas al 100% de la participación en trabajo de cuidados no remunerado (respuesta 'Sí') según condición de actividad, en dos paneles separados por sexo. Entre los hombres, la mayor participación se da en los ocupados (36,6%), bajando a 22,3% en quienes están fuera de la fuerza de trabajo. Entre las mujeres el patrón es distinto: la participación es alta tanto en ocupadas (47,8%) como en quienes están fuera de la fuerza de trabajo (36,7%), y aún mayor entre las desocupadas (50,6%)."
enut_practico %>%
  ...
  scale_fill_manual(name = "Participa en Cuidados", values = c("#1E2240", "#7C90CC")) +
  labs(
